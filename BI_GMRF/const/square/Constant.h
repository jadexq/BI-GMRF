...
public:
    ...
    // zeta_alpha
    double aIA = -2.556; // a_{I}^{alpha} <<
    double bIA = 0.778; // b_{I}^{alpha} <<
    // zeta_beta
    double aIB = -1.133; // a_{I}^{beta} <<
    double bIB = 0.917; // b_{I}^{beta} <<
    // association between zeta_alpha and zeta_beta
    double cI = 0.130; // c_I <<
    // * GMRF (alpha and beta)
    double sig1Alpha = 0.7; // sigma_{alpha 0} <<
    double sig1Beta = 0.083; // sigma_{beta 0} <<
    double sigDFix = 0.98; // sigma_{delta} <<
    // * gamma, normal prior
    double mu0Gamma = 0;
