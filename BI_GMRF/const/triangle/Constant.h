...
public:
    ...
    // zeta_alpha
    double aIA = -3; // a_{I}^{alpha} <<
    double bIA = 0.667; // b_{I}^{alpha} <<
    // zeta_beta
    ...
    double cI = 0.115; // c_I <<
    // * GMRF (alpha and beta)
    double sig1Alpha = 0.20; // sigma_{alpha 0} <<
    double sig1Beta = 0.067; // sigma_{beta 0} <<
    double sigDFix = 0.97; // sigma_{delta} <<
    // * gamma, normal prior
    double mu0Gamma = 0;
